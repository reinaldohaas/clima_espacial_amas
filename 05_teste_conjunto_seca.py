...
jz=pd.read_csv("rede_completa_14_estacoes_currents_2024.csv",parse_dates=["datetime"]).set_index("datetime").sort_index()
jz=jz.loc[~((jz.index.hour==0)&(jz.index.minute==0))]
jzd=jz.resample("1D").agg(Jz=("Jz_pA_m2",lambda s:np.sqrt((s**2).mean())))
kpp=f"{OUT}/kp_dst_2024.csv"; kpp=kpp if os.path.exists(kpp) else "kp_dst_2024.csv"
